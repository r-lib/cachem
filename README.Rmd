...
The **cacheback** R package provides objects creating and managing caches. These cache objects are key-value stores, but unlike other basic key-value stores, they have built-in support for memory and age limits so that they won't have unbounded growth.

The cache objects in **cacheback** differ from some other key-value stores in the following ways:
...
**Cacheback** comes with two kinds of cache objects: a memory cache, and a disk cache.
...
The cache objects provided by cacheback have automatic pruning. (Note that pruning is not required by the API, so one could implement an API-compatible cache without pruning.)
